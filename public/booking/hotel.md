...
#####Request Parameters
  ...
  `blocks[].[id]` | `str|int` | любой уникальный id блока. если кол-во номеров с одинаковым тарифом больше одного. Если указан, то в ответе от сервера в параметре [`booking`](#f_booking) в блоках этот id так же будет присутствовать (не используется системой, может понадобится только для привязки для валидации)
