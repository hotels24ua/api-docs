...
#####Parameter `continue`
Поле [`continue`](#f_continue) используется для продолжения бронирования с определенной стадии.
...
Далее приложение подтверждает бронирование. Для этого используется всего два поля в запросе: [`request_id`](#rf_request_id) и [`continue`](#f_continue) со значением `true`
